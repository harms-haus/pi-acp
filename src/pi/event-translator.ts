...
import { piContentToAcpBlocks, toolNameToKind, kindToTitle, piToolResultToAcpContent, } from "../utils/content-translation.js";
import { generateTurnId } from "../utils/turn-id.js";

import { getTurnId, setTurnId, isSessionCancelling, setSessionCancelling, getPromptRequestId, setPromptRequestId } from "./session-registry.js";
...
export function handlePiEvent(event: AgentSessionEvent, sessionId: SessionId): void {
  switch (event.type) {
    case "agent_start": {
      // New turn — generate a turn ID
      const turnId = generateTurnId();
      setTurnId(sessionId, turnId);
      break;
    }
    case "agent_end": {
      // Turn ended — check if we should resolve the pending prompt
    ...
    case "tool_execution_update": {
      ...
      const content = event.partialResult !== undefined ? piToolResultToAcpContent(event.toolName, event.partialResult) : undefined;
      sendUpdate(sessionId, {
        sessionUpdate: "tool_call_update",
        toolCallId: event.toolCallId,
        status: "in_progress",
    ...
    case "turn_start": {
      // Turn started — we may already have a turnId from agent_start
      // If not, generate one
    ...
    case "turn_end": {
      // Turn ended — clear tool call tracking for completed tools
    ...
    case "compaction_start":
      // Could send a plan update showing compaction in progress
      break;
    case "compaction_end":
      break;
    default:
      // Ignore other event types (queue_update, session_info_changed, etc.)
      break;
  }
}
